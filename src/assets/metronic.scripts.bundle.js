    ...
    $.fn.mDatatable = function (options) {
        ...
        var dt = {
            ...
            init: function (options) {
                ...
                //get no record found text i18n from appcomonent.html
                const noRecordText=$("#noRecordFoundDatatable").text();
                options['noRecordText']=noRecordText;
            ...
            getData: function () {
                ...
                if (options.data.type === 'remote') {
                    ...
                    params.contentType = API.getOption('data.source.read.contentType') || params.contentType;
                    params.data = API.getOption('data.source.read.params');
                    // params.data['datatable'] = API.getDataSourceParam();

                    var reqParams = API.getDataSourceParam();
                    params.data.skip = (API.getDataSourceParam('pagination').page - 1) * params.data.take;
                    if (reqParams['sort']['sort']) {
                        params.data.sort = {
                            direction: reqParams['sort']['sort'].charAt(0).toUpperCase() + reqParams['sort']['sort'].slice(1),
                            fieldName: reqParams['sort']['field'].charAt(0).toUpperCase() + reqParams['sort']['field'].slice(1)
                        }
                    }
                    ...
                    // }
                    var loadDataFunc = API.getOption('data.source.read.loadDataFunc');
                }
                ...
                if (loadDataFunc) {
                    deferred = $.Deferred();
                    var a = function () {
                        loadDataFunc(params.data)
                            .then(function (result) {
                                if (result && result.data && result.data.data) {
                                    if (result.data.data.length === 0 && result.data.totalRecord > 0 && params.data.skip > 0) {
                                        params.data.skip = Math.max(0, params.data.skip - result.data.take);
                ...
                return deferred
                    .then(function (result) {
                        ...
                        if (result.data.data) {
                            var pages = Math.ceil(result.data.totalRecord / result.data.take);
                            return {
                                data: result.data.data,
                                meta: {
                                    page: result.data.skip / result.data.take + 1,
                                    pages: pages,
            ...
            spinnerCallback: function (block) {
                ...
                if (spinnerOptions.disabled) return;
                if (block) {
                    if (!dt.isSpinning) {
    ...
    $.fn.mDatatable.defaults = {
        ...
        translate: {
            records: {
                ...
                noRecords: ()=>{
                    const text = API.getOption('optionNoRecordFound') ?  API.getOption('optionNoRecordFound') : $("#noRecordFoundDatatable");
...
(function ($) {
    ...
    $(document).on('click', function(e) {
        $('.m-dropdown.m-dropdown--open').each(function() {
          ...
          $(".exchange-view-table").on("init-ads-table",() => {
              if(!$(".exchange-view-table.offer .mCustomScrollBox").length)
              mApp.initScrollBar();
              if(!$(".exchange-view-table.bid .mCustomScrollBox").length)
              mApp.initScrollBar();
          });
    });
}(jQuery));
...
var mLayout = function() {
    ...
    var initQuicksearch = function() {
        ...
        qs.mQuicksearch({
            ...
            templates: {
                error: function(qs) {
                    return '<div class="m-search-results m-search-results--skin-light"><span class="m-search-result__message">Connection unsuccessful</div></div>';
                }
            }
        });
    }
